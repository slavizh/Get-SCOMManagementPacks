...
param (
    ...
    [parameter(Mandatory=$false)]
    [string]$MPLogFileName = "MPUpdates.log",

    [parameter(Mandatory=$false)]
    [string]$MPErrorLogFileName = "MPErrorLog.log",

    [parameter(Mandatory=$false)]
    [bool]$ReDownloadMissingFiles = $false,

    [parameter(Mandatory=$false,ParameterSetName="AgeDate")]
    [datetime]$SkipMPsOlderThanDate,

    [parameter(Mandatory=$false,ParameterSetName="AgeMonths")]
    [int]$SkipMPsOlderThanMonths,
...
#region Variables
[string]$MPWikiUri = "http://social.technet.microsoft.com/wiki/contents/articles/16174.microsoft-management-packs.aspx"
...
function Write-CMTraceLog {
    ...
    param (
        ...
        [string]$Message,
        [Parameter(Mandatory=$false)]
        [string]$ModuleName = "Get-SCOMManagementPacks",
        [Parameter(Mandatory=$false)]
        [string]$LogFile = "D:\MPs\MPUpdates.log"
    )
...
if ((Test-Path -Path $MPPath) -eq $true) {
    ...
    [string]$MPPath = New-Item -ItemType Directory -Path $MPPath -Force -ErrorAction Stop | Select-Object -ExpandProperty FullName
}
...
if ($SkipMPsOlderThanDate) {
    ## Filter date supplied, calculate the number of days between the supplied date and today
    [int]$AgeFilter = New-TimeSpan -Start $SkipMPsOlderThanDate -End $StartTime | Select-Object -ExpandProperty Days
}
elseif ($SkipMPsOlderThanMonths) {
    ## Filter months supplied, calculate the number of days between the supplied number of months ago and today
    [int]$AgeFilter = New-TimeSpan -Start ($StartTime.AddMonths(-$SkipMPsOlderThanMonths)) -End $StartTime | Select-Object -ExpandProperty Days
}
else {
